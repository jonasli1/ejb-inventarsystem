...
struct LoanModelsTests {
    ...
    @Test func decodesCalendarEntry() throws {
        let json = Data("""
        {"id":"d36ce48b-7346-4554-9bde-98f333a38a73","subject":"Jugendfreizeit","borrowerName":"Max Mustermann","borrowerPersonId":null,"status":"completed","checkoutDate":"2026-09-12T20:23:04.702Z","dueDate":"2026-10-01T00:00:00.000Z","itemCount":1}
        """.utf8)

        let entry = try APICoding.decoder.decode(CalendarLoanEntry.self, from: json)
        #expect(entry.status == .completed)
        #expect(entry.itemCount == 1)
        #expect(entry.subject == "Jugendfreizeit")
    }

    @Test func createLoanInputEncodesSaveAsTemplateOnlyWhenNamed() throws {
        var input = CreateLoanInput(
            subject: "Jugendfreizeit", borrowerPersonId: nil, borrowerName: "X", borrowerStreet: "S", borrowerCity: "C",

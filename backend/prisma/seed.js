...
const permissions_1 = require("../src/common/constants/permissions");
const prisma = new client_1.PrismaClient();
const ROLE_DEFINITIONS = [
    {
        name: 'Admin',
        description: 'Full system access',
        permissionKeys: permissions_1.ALL_PERMISSIONS.map((p) => p.key),
    },
    {
        name: 'Lagerwart',
...
async function main() {
    ...
    const permissionRecords = await Promise.all(permissions_1.ALL_PERMISSIONS.map((p) => prisma.permission.upsert({
        where: { key: p.key },
    ...
    const mischpult = await prisma.article.upsert({
        ...
        create: {
            ...
            type: client_1.ArticleType.UNIQUE,
    ...
    const stromkabel = await prisma.article.upsert({
        ...
        create: {
            ...
            categoryId: categoryTechnik.id,
            type: client_1.ArticleType.BULK,
            unitOfMeasure: 'Stück',
        },
    });
    const gaffaTape = await prisma.article.upsert({
        ...
        create: {
            ...
            categoryId: categoryTechnik.id,
            type: client_1.ArticleType.CONSUMABLE,
            unitOfMeasure: 'Rolle',
        },
    });
    ...
    await prisma.userGroup.upsert({
        ...
        update: {},
        create: { userId: adminUser.id, groupId: groupVorstand.id, source: client_1.GroupSource.manual },
    });
